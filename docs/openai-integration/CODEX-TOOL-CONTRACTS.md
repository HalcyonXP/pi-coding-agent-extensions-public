...
Pi's Unified exec still returns JSON text plus structured `details`, and nested callers receive the native `{result,isError}` wrapper. At the audit baseline, Code exec/wait also returned a JSON envelope; PR #15's readable TUI did not alter it. The subsequent [direct Code-output transition](NATIVE-CODE-CONTRACT.md) changes only exec/wait model content and adds measured per-call metadata in details, with explicit SDK migration and historical rendering. Aligning tool-specific nested results or direct Unified exec formatting still needs separate native/result-projection design, protected-evidence and compatibility tests—not another visual renderer or speculative duplicate fields. Nonzero process exits, tool errors, hook feedback, OS completion and successful return are separate observations. Exact error text, every unknown-field/hook path, and the full truncation algorithms are not yet certified equivalent.
...
1. Native freeform input and the direct Code text/details transition are described separately and require their own source/artifact acceptance. Current SDK consumers use details; historical messages and nested `{result,isError}` consumers remain intact. Next design tool-specific nested projection and direct Unified exec output without conflating them with Code status formatting.
