...
## Working-source native notifications

`await notify(value)` accepts exactly one primitive; serialize arrays/objects explicitly. It submits bounded literal text through the original exec's protected native scope, retaining that identity across adopted waits. Nonblank output requires an active native turn; whitespace-only native output is a no-op. It never starts an idle turn or falls back to `text()`/a toast. Await queue acceptance; detached operations fail rather than becoming implicit background work. Do not replay effects after a failure.

Notifications are additional native tool-result records, mapped by the existing provider's custom/function tool support. Custom-message labels, lookup IDs and ordinary tool-return fields cannot mint this channel. Native journal and UI delivery follow the initial result; separate cards retain the original call identity without overwriting its result or emitting another tool-completion/usage event. Additional result records count as results, not additional tool calls. SDK consumers must distinguish top-level `notification: true` records from exec/wait completion envelopes; `readCodeOutcome` explicitly refuses notifications. Orphaned history stays labelled historical text rather than inventing a provider call or restoring live authority.

Each notification is at most65536 UTF-8 bytes and shares the native64-publication/four-pending quota. Queue acceptance is not model consumption or confirmed cleanup. Revocation can quarantine delivery while retaining the journal; uncertain dispatched publication keeps draining ownership. `NOTIFY_VALUE_UNSUPPORTED`, `NOTIFY_INACTIVE` and `NOTIFY_UNAVAILABLE` are fixed feedback, not inspected guest exception text. Matched package pins and source development checks do not replace separate source/master/copied-artifact acceptance.
