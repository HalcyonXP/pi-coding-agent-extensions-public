...
| Code mode | [Native raw-JavaScript grammar input](NATIVE-CODE-CONTRACT.md) implemented through existing Pi API, with explicit pragma-only model/RPC controls and CRLF repair; this is not exact-head acceptance. Direct JSON/nested wrappers remain unchanged. Runtime/helper implementation, notification ordering, result projection and lifecycle adaptation remain work. Preserve restricted coordination, native delegation, original cell ownership and confirmed cleanup. |
