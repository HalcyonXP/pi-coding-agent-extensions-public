...
Pinned basis: [exposed tool contracts](CODEX-TOOL-CONTRACTS.md). Numeric IDs and Unified default/wait transitions now have their own [live contract](NATIVE-UNIFIED-CONTRACT.md). [Opt-in Unified projections](CODE-RESULT-PROJECTION.md) preserve existing `tools` wrappers; default namespace parity, other tool-specific projections, broader normalized naming, notify ordering, inline media and broader Web/image/context/lifecycle parity remain open. No additional native authority, provider fallback, PTY or hosted-service guarantee is introduced.
