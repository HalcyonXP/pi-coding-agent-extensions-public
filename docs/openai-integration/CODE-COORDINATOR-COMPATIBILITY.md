...
Pinned basis: [exposed tool contracts](CODEX-TOOL-CONTRACTS.md). Numeric IDs and Unified default/wait transitions now have their own [live contract](NATIVE-UNIFIED-CONTRACT.md). [Default Unified projections](CODE-RESULT-PROJECTION.md) now use `tools`, with explicit `nativeTools` raw access and the retained `projectedTools` alias. Legacy wrapper scripts need migration; full default-API parity, other tool-specific projections, broader normalized naming, notify ordering, inline media and broader Web/image/context/lifecycle parity remain open. No additional native authority, provider fallback, PTY or hosted-service guarantee is introduced.
