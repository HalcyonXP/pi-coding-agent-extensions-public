...
A **Web admission profile** selects the extension's Web argument/schema contract. It is not a Pi installation profile, a capability switch, a credential or service entitlement. PR #33 accepted the bounded context-free profiles through separate source/master/copied-handoff gates. PR #34 separately accepted the bounded conversation-disclosure profile below. See the [milestone index](MILESTONES.md) for exact accepted sources and masters; these checkpoints do not certify later working changes.
...
## Evidence and per-artifact gates
