...
## Background wait preference

`/openai-tools background-wait [status | <milliseconds>]` and the native **Background wait ceiling** settings row control future empty-input polls. Accept canonical decimal CLI text and integer JSON values from5,000 to300,000 ms; no coercion, zero, fractions, exponents or larger budget. The TUI offers presets and retains any valid custom CLI value. Setting a ceiling does not enable a capability, cancel a job or change an already pending collection's timer. Direct and nested handlers snapshot the trusted policy once per invocation; native manager limits remain independent.

The separate profile file `openai-compatibility-unified.json` contains exactly `{ "version": 1, "maxBackgroundWaitMs": 60000 }` (example). Missing files use300,000 without writes; fresh sessions/reload restore the preference, not jobs or authority. It does not rewrite the four-boolean capability file or Fast file, and is not live cross-process synchronization. Invalid/oversized/unknown-version state is preserved and prevents Unified activation; other capabilities and owned Jobs cancellation remain separate. Repair the file and reload. A failed explicit save leaves the current effective ceiling unchanged; atomic replacement is not a cross-process transaction or hostile-filesystem sandbox.

The source implementation and focused tests are not yet installed-artifact acceptance for this batch. Controlled timer tests verify exact ceilings, snapshot behavior and unchanged cancellation semantics. Extended installed wait acceptance uses the existing22 synthetic requests, a60,000 ms configured ceiling and invalid-setting/readback checks while retaining the original job; it must pass independently at the batch milestone.
