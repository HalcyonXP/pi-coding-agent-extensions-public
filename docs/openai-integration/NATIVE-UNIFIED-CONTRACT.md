...
These replace the historical1,000 ms/4,096-token defaults, not explicit controls. Explicit integer waits0–30,000 ms (including zero) and output1–16,384 tokens remain supported. This intentionally does not implement the inspected upstream explicit wait floors or300,000 ms background maximum. Return wait is not CPU/process lifetime, readiness or retention; no execution or aggregate budget is widened. Native hooks finalize normally and can still cause refusal. Code exec/wait defaults are separate and unchanged.
