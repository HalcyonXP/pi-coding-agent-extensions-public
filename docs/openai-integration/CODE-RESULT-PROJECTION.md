...
The direct Web result adds `details.web_result: {version:1,sha256}`: a **consistency stamp** over content and verification/source-presence labels, not a signature or authority. Native `CellEvidence` checks the finalized shape/stamp and only after successful publication supplies `protected_evidence.projection: "web-text-v1"`. Guest code cannot supply this native admission decision. Ordinary hook changes invalidate the stamp or shape and preserve the full wrapper; trusted installed JavaScript is not sandboxed by a public checksum.

`nativeTools.web_search` retains the complete original-call wrapper, including protected evidence metadata. Choose it **before the original invocation** if metadata or bounded `evidence(ref,...)` views are needed; do not repeat a service operation merely to obtain another presentation. For the default view, use `typeof r === "string"`; otherwise inspect the complete wrapper. Oversized replies retain the existing explicit evidence-view fallback with no text-projection hint. A smaller string does not admit an otherwise oversized native wrapper.
