...
The current acceptance path still requires a real returned shell's READY marker, successful cell result, automatic scope-owned teardown without another wait, and zero final active/draining scopes. Runtime limits, native supervisor admission and bounded polling remain intact. The diagnostic allowlist must include both base runtime and RPC error codes, otherwise a future `TOOL_LIMIT`/`GATEWAY_FAILED` would be hidden as unclassified. Do not print guest output, private paths or payloads to diagnose it.
