...
From the repository root, install openai-compatibility dependencies with npm ci --ignore-scripts --prefix openai-compatibility. On Windows explicitly prepare pinned compiler inputs and build the verified helper first. Then run node --test experiments/code-mode-runtime/test/*.test.mjs. Genuine AgentSession/QuickJS integration additionally needs the prepared native host and .github/scripts/test-code-mode-rpc.mjs --rpc; see [validation](../../docs/openai-integration/VALIDATION.md).
