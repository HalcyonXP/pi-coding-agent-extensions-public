...
for(const key of ['type','mimeType','ref'])test('image helper refuses a reference accessor without calling it: '+key,async()=>{
 let effects=0;const output=[];const code=`let reads=0;const value={type:"image_reference",mimeType:"image/png",ref:"img_11111111-1111-1111-1111-111111111111"};Object.defineProperty(value,${JSON.stringify(key)},{get(){reads++;throw Error("getter executed");}});try{image(value)}catch{}text(reads);`;
 const r=await evaluate(code,{cell:{output:v=>output.push(v),yield(){},pump(){},operation(){},syncOperation(){effects++;return{};}}});assert.equal(r.status,'ok');assert.deepEqual(output,['0']);assert.equal(effects,0);
});
test('image helper ignores inherited reference getters when accepting exact inline data',async()=>{
 const operations=[],output=[];const r=await evaluate(`let reads=0;Object.defineProperty(Object.prototype,"type",{get(){reads++;throw Error("inherited getter executed");}});image({image_url:${JSON.stringify('data:image/png;base64,'+png)}});text(reads);`,{cell:{output:v=>output.push(v),yield(){},pump(){},operation(){},syncOperation:v=>{operations.push(v);return{published:true};}}});assert.equal(r.status,'ok',JSON.stringify(r));assert.deepEqual(output,['0']);assert.deepEqual(operations,[op()]);
});

for(const[name,Runtime]of [['semantic',CellRuntime],['contained Windows',WindowsCellRuntime]])test(name+' image form selection never calls reference getters and preserves owned forwarding',{skip:name==='contained Windows'&&process.platform!=='win32'},async()=>{
 const published=[],f=setup(async e=>published.push(e)),store=new CellStore(),runtime=new Runtime({store,evidence:f.broker,output(){throw Error('No guest text');},yield(){}}),failures=[];
 try{const r=await runtime.run(`let reads=0;const url=${JSON.stringify('data:image/png;base64,'+png)};const first=image(url);for(const key of ["type","mimeType","ref"]){const value={type:"image_reference",mimeType:"image/png",ref:first.ref};Object.defineProperty(value,key,{get(){reads++;throw Error("getter executed");}});try{image(value)}catch{}}Object.getOwnPropertyDescriptor=()=>{throw Error("replacement descriptor called");};image({type:"image_reference",mimeType:"image/png",ref:first.ref,bytes:1});Object.defineProperty(Object.prototype,"type",{get(){reads++;throw Error("inherited getter executed");}});image({image_url:url});if(reads!==0)throw Error("guest getter executed");`,{gateway:f.scope,allowedTools:[],signal:AbortSignal.timeout(5000)});assert.equal(r.status,'ok',JSON.stringify(r));assert.equal(published.length,3);for(const p of published)assert.deepEqual(p.content,[{type:'image',data:png,mimeType:'image/png'},{type:'text',text:INLINE_IMAGE_LABEL}]);assert.equal(published[1].details.repeated,true);}
 catch(e){failures.push(e);}finally{try{await runtime.close();}catch(e){failures.push(e);}f.broker.close();store.close();}if(failures.length)throw new AggregateError(failures,'Image descriptor selection or cleanup failed');
});
