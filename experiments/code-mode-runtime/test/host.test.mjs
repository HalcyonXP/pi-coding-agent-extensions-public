...
test("missing engine in a path with spaces is reported, not replaced by Node eval", async () => {
  ...
  await withFixtureCleanup(async () => {
    for (const file of ["worker.mjs", "protocol.mjs", "engine.mjs", "evaluator.mjs", "rpc-protocol.mjs", "cell-bootstrap.mjs", "cell-protocol.mjs", "tool-metadata.mjs", "cell-helper-errors.mjs"]) await cp(new URL(`../../../openai-compatibility/runtime/${file}`, import.meta.url), join(directory, file));
