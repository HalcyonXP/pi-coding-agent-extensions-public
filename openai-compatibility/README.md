...
Direct jobs remain pollable in their original conversation/native context while other ordinary tools run. Cell jobs additionally require the original cell scope and close when that cell completes. IDs cannot adopt a different context/cell. Model/session/registry changes, abort/reload and expiry can revoke jobs; keeping the terminal open does not prove the original native context remains current. Native scopes last at most six minutes, the shell manager has a five-minute idle and ten-minute total ceiling, and completed buffers expire after roughly a minute without collection. Collect promptly; a terminal result without a `session_id` has been consumed.

There is **no automatic completion message or new assistant turn** for these shell jobs. The assistant must poll to collect their output; a passive Jobs snapshot is not a notification subscription. Protected evidence persistence is a separate mechanism, not an automatic live model request. If polling reports unknown/expired/foreign, preserve the call timeline, redacted arguments/results and passive settings/Jobs state. That generic error alone does not identify the cause or establish which OS effects occurred.
