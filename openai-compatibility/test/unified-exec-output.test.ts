...
test("running startup remains unconfirmed; readiness never implies command success", () => {
 const r = directUnifiedResult(sample({ session_id: "synthetic-job", running: true, exit_code: null, supervisor_ready: false, output: "" }), 0);
 assert.equal(r.content[0].text, 'Wall time: 0.0000 seconds\nProcess running with session ID synthetic-job\nSupervisor preamble: not confirmed\nOutput:\n');
...
test("an exited retained ID denotes remaining output, not a running process or new authority", () => {
 const r = directUnifiedResult(sample({ session_id: "synthetic-retained", exit_code: 7, supervisor_ready: true }), 214);
 assert.match(r.content[0].text, /^Wall time: 0\.2140 seconds\nProcess exited with code 7\nMore output available with session ID synthetic-retained\nSupervisor preamble: received\nOutput:/);
...
test("an unconfirmed OS termination still reports the physically running process", () => {
 const r = directUnifiedResult(sample({ session_id: "synthetic-draining", running: true, exit_code: null, termination: "OS process termination was not confirmed; further launches are blocked after reset." }), 2000);
 assert.match(r.content[0].text, /Process running with session ID synthetic-draining/); assert.match(r.content[0].text, /Termination: OS process termination was not confirmed/);
...
for (const [name, alter] of Object.entries({
 ...
 'contradictory running exit': (v: any) => { v.running = true; v.session_id = 'synthetic'; v.exit_code = 7; },
 'numeric ID': (v: any) => { v.session_id = 123; },
...
test("factory measures each awaited direct manager call; native failures, arguments and leases stay native", async t => {
 const calls: unknown[][] = [], values = [sample({ session_id: 'synthetic-job', running: true, exit_code: null }), sample()]; let releases = 0;
...
test("genuine-invocation-shaped nested scope keeps JSON/details and does not measure or publish direct output", async t => {
 ...
 const a = await tools[0].execute('start', { cmd: 'not executed' }, undefined, undefined, ctx), b = await tools[1].execute('poll', { session_id: 'synthetic-owned' }, undefined, undefined, ctx);
...
test("invalid or revoked native ownership refuses before manager or timing; guest data cannot choose direct formatting", async t => {
 ...
 for (const toolsContext of [{ origin: 'nested', contextSignal: new AbortController().signal }, { origin: 'invented', contextSignal: new AbortController().signal }, { origin: 'direct', contextSignal: aborted.signal }]) {
  ...
  await assert.rejects(tools[1].execute('poll', { session_id: 'synthetic' }, undefined, undefined, ctx), /scope|metadata|revoked/);
 }
...
for (const api of ['openai-responses', 'openai-codex-responses'] as const) for (const name of ['exec_command', 'write_stdin'] as const) test(api + ' replays native ' + name + ' text without details or history mutation', () => {
 ...
 const id = 'call_unified|fc_unified', assistant: AssistantMessage = { role: 'assistant', content: [{ type: 'toolCall', id, name, arguments: name === 'exec_command' ? { cmd: 'not executed' } : { session_id: 'synthetic' } }], api, provider: model.provider, model: model.id, usage: { input: 1, output: 1, cacheRead: 0, cacheWrite: 0, totalTokens: 2, cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0, total: 0 } }, stopReason: 'toolUse', timestamp: 1 };
