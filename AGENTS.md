...
- Code's `tools.*` keeps native `{result,isError}`. Opt-in `projectedTools.*` uses the same names/aliases and dispatch; `runtime/tool-result-projection.mjs` only projects recognized normal Unified data after the unchanged RPC/evidence boundary. Errors, loss, hook changes and protected evidence retain wrappers. Private bridge timing is not process runtime; this is not default Codex API parity. See `docs/openai-integration/CODE-RESULT-PROJECTION.md`.
