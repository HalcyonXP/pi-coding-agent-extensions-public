...
export async function acceptQuietPolling(session,invoke,outcome,faux,ai,native) {
 ...
 for(const cancel of [false,true]){
  ...
  try{
   ...
   const code=`const jobs=await Promise.all(${JSON.stringify(durations)}.map(async(ms,n)=>{const r=await nativeTools.exec_command({cmd:'Start-Sleep -Milliseconds '+ms+'; Write-Output QUIET_TIMER_'+n,yield_time_ms:1});if(r.isError||!r.result.details.running)throw Error('synthetic start refused');const id=r.result.details.session_id;if(!Number.isInteger(id)||id<1||id>2147483647)throw Error('numeric ID missing');return {id,n};}));yield_control();for(const job of jobs){let r,output='';do{r=await nativeTools.write_stdin({session_id:job.id,yield_time_ms:250});if(r.isError)throw Error('synthetic poll refused');output+=r.result.details.output;}while(r.result.details.running);if(r.result.details.exit_code!==0||!output.includes('QUIET_TIMER_'+job.n))throw Error('synthetic output missing');text('QUIET_DONE_'+job.n);}`;
   ...
   for(const e of quiet){const id=e.result.details.session_id;assert.ok(Number.isInteger(id)&&id>0&&id<=2147483647);const start=raw.find(s=>s.type==="tool_execution_start"&&s.toolCallId===e.toolCallId);assert.equal(start.args.session_id,id);}
