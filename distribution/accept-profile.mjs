...
assert.equal(settingsMenu.status,"passed");assert.equal(settingsMenu.networkAttempts,0);assert.equal(settingsMenu.normalAndExcludedContexts,true);assert.equal(settingsMenu.consolidatedJobs,true);assert.equal(settingsMenu.savedCapabilityPreferences,true);assert.equal(settingsMenu.quietPollingPresentation,true);validatePollingFrames(settingsMenu.frames);
