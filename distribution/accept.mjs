...
let requests=0,auth=0,session,faux,codeUI;
...
try{
 ...
 r=outcome(await invoke("exec",{code:'text(load("artifact"));yield_control();await new Promise(r=>setTimeout(r,10000));',yield_time_ms:10000}));assert.equal(r.status,"running");assert.deepEqual(r.output,["7"]);const oldId=r.cell_id;await codeUI.settle();inspectCodeResult(codeUI,results.at(-1),'running','7');
