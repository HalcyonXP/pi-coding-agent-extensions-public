...
try{
 ...
 r=outcome(await invoke("exec",{code:'// @exec: {"max_output_tokens":0}\nconst r=await tools.imagegen({prompt:"synthetic reference edit",referenced_image_refs:[load("image")],destination_path:"artifact-edit.png"});if(r.isError)throw Error("edit denied");'}));assert.equal(r.status,"completed");assert.equal(requests,3);assert.equal(auth,3);
