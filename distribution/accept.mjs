...
import {acceptUnifiedDefaults} from "./accept-unified-defaults.mjs";
...
try{
 ...
 r=outcome(await invoke("exec",{code:'// @exec: {"max_output_tokens":0}\nawait tools.web_search({search_query:[{q:"synthetic public artifact"}]});const r=await tools.imagegen({prompt:"synthetic artifact PNG",destination_path:"artifact.png"});if(r.isError||r.result.content[0].data)throw Error("projection failed");store("image",r.result.content[0].ref);text("suppressed marker");'}));assert.equal(r.status,"completed");assert.deepEqual(r.output,[]);await codeUI.settle();inspectCodeResult(codeUI,results.at(-1),'completed','Output omitted:');
 r=outcome(await invoke("exec",{code:'// @exec: {"max_output_tokens":0}\nconst r=await tools.imagegen({prompt:"synthetic reference edit",referenced_image_refs:[load("image")],destination_path:"artifact-edit.png"});if(r.isError)throw Error("edit denied");image(r.result.content[0]);'}));assert.equal(r.status,"completed");assert.equal(r.result.status,"ok");assert.equal(requests,3);assert.equal(auth,3);
 ...
 const nativeUnifiedDefaults=await acceptUnifiedDefaults(session,runtime,nativeStream,cwd);
