...
try{
 ...
 r=outcome(await invoke("exec",{code:'const = "synthetic diagnostic detail"',max_output_tokens:0}));assert.equal(r.status,"completed");assert.equal(r.result.code,"EXECUTION_FAILED");assert.equal(r.result.diagnostics.guest_phase,"compile");assert.equal(r.result.diagnostics.delegated_calls,0);assert.deepEqual(r.output,[]);
