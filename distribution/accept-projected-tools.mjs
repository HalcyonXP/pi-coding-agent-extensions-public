...
export function validateProjectedTools(rows){
 ...
 for(const r of rows){assert.equal(r.requests,8);assert.equal(r.scenarios,4);for(const key of ['targetFields','defaultUnifiedProjection','legacyWrappersPreserved','projectionAliasPreserved','numericRetainedID','completeOutput','nativeWrappersUnchanged','hookFallbackVisible','bridgeTimeBounded','nativeCustomReplay','historyUnchanged'])assert.equal(r[key],true);assert.equal(r.activeScopes,0);assert.equal(r.drainingScopes,0);assert.equal(r.liveServiceCalls,0);}
 return {defaultUnifiedProjection:true,legacyWrappersPreserved:true,projectionAliasPreserved:true,numericRetainedID:true,hookFallbackVisible:true,bridgeTimeObserved:true,nativeScopesReleased:true,syntheticModelRequests:16,scenarios:8,liveServiceCalls:0};
}
 ...
 try{
  ...
  for(const provider of ['openai','openai-codex']){
   ...
   for(const scenario of ['complete','retained','hook','raw']){
    ...
    const api=scenario==='raw'?'nativeTools':scenario==='retained'?'projectedTools':'tools';
    const source='// @exec: {"yield_time_ms":30000,"max_output_tokens":3000}\n'+`let r=await ${api}.exec_command({cmd:${JSON.stringify(command)},max_output_tokens:${scenario==='retained'?4:100}});`+(hook||scenario==='raw'?'text(JSON.stringify(r));':`for(let n=0;;n++){if(n>=32||typeof r.output!=="string")throw Error("projection unavailable");text(JSON.stringify(r));if(!r.session_id)break;r=await ${api}.write_stdin({session_id:r.session_id,max_output_tokens:4});}`);
   ...
   rows.push({api:model.api,requests:total,scenarios:4,targetFields:true,defaultUnifiedProjection:true,legacyWrappersPreserved:true,projectionAliasPreserved:true,numericRetainedID:true,completeOutput:true,nativeWrappersUnchanged:true,hookFallbackVisible:true,bridgeTimeBounded:true,nativeCustomReplay:true,historyUnchanged:true,activeScopes:0,drainingScopes:0,liveServiceCalls:0});
  }
