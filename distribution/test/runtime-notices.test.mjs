...
test("canonical runtime notices retain pinned full grants, source witnesses and exact ranges",async()=>{
 const bytes=await readFile(new URL("../supplemental-notices.json",import.meta.url)),p=parseSupplementalNotices(bytes),entries=p.entries.filter(e=>e.coverage.kind==="runtime-notice");assert.equal(entries.length,46);assert.equal(supplementalNoticeFiles(bytes).length,93);
 assert.equal(entries.filter(e=>e.coverage.noticeSource.kind==="excerpt").length,43);
 ...
 for(const e of entries){const notice=await readFile(new URL("../../"+e.noticePath,import.meta.url));assert.equal(notice.length,e.noticeBytes);assert.equal(sha256(notice),e.noticeSha256);assert.deepEqual(inspectText(notice.toString(),e.noticePath),[]);let witness=notice;
  ...
  assert.ok(e.coverage.consumers.every(f=>f.path.startsWith(e.packagePath+"/dist/")));assert.equal(e.coverage.consumers[0].sha256,"105c3bed22d457e43e3d1c3c1c6959fda62a8fe06f0fc8a985303c3a2be72232");
 }
