...
for(const [name,alter]of Object.entries({
 ...
 'numeric ID':r=>{r.details.session_id=1;},
...
test('running and unknown-exit zero-output direct results remain distinct',()=>{
 const r=message();r.details={output:'',exit_code:null,running:true,session_id:'synthetic',truncated_bytes:0,unified_result:{version:1,wall_time_ms:0}};r.content[0].text='Wall time: 0.0000 seconds\nProcess running with session ID synthetic\nOutput:\n';readUnifiedOutcome(r);
...
for(const field of ['requests','nativeFunctionReplay','sameOwnedSession','literalReturnedOutput','exitedRetainedOutput','nonzeroExitAndLossVisible','readinessVisible','nativeDirectPresentation','measuredCallTimeBoundedByObservation','unchangedHistory','activeScopes','drainingScopes','liveServiceCalls'])test('missing native observation '+field+' refuses acceptance',()=>{const r=rows();delete r[1][field];assert.throws(()=>validateNativeUnifiedOutput(r));});
