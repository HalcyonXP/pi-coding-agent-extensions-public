 ...
 try{
  ...
  for(const provider of ['openai','openai-codex']){
   ...
   for(const stage of ['start','finish','drain','loss']){
    ...
    if(stage==='start'){assert.equal(value.running,true);assert.equal(value.exit_code,null);assert.equal(value.output,UNIFIED_BEFORE);assert.equal(value.truncated_bytes,0);assert.equal(typeof value.session_id,'string');cell=value.session_id;}
    ...
    else {assert.equal(value.running,false);assert.equal(value.exit_code,0);assert.equal(value.output,'x'.repeat(16));assert.equal(value.truncated_bytes,1024*1024);assert.equal(typeof value.session_id,'string');assert.match(message.content[0].text,/Output omitted: 1048576 bytes/);await session.prompt(`/openai-tools jobs cancel ${value.session_id}`);}
